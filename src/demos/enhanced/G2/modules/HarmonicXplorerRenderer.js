...
class HarmonicXplorerRenderer extends HarmonicXplorerModule {
    ...
    // Draw a visualization of the harmonic series and relationships
    drawHarmonicRepresentation() {
        try {
            ...
            for (let i = 0; i < harmonics.length; i++) {
                for (let j = i + 1; j < harmonics.length; j++) {
                    if (j === i * 2 || i === j * 2 || j === i * 3 || i === j * 3) {
                        // Draw relationships between octaves and simple ratios
            ...
            // Draw each harmonic as a point with label
            for (let i = 0; i < harmonics.length; i++) {
                const r = (i + 1) * spacing;
                const harmonic = harmonics[i];
                const angle = harmonic.ratio * 0.1; // Spread points around a spiral
