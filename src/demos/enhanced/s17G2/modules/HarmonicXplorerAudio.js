...
class HarmonicXplorerAudio extends HarmonicXplorerModule {
    ...
    createOscillators() {
        try {
            ...
            // Get state values
            const { baseFrequency, oscillatorType, audioAttack, audioDecay } = this.core.state;
            ...
            this.oscillators = harmonics.map((harmonic, index) => {
                ...
                // Apply amplitude scaling to higher harmonics (1/n falloff)
                const amplitude = 1 / (index + 1) * 0.5;
    ...
    // Create chords based on harmonic relationships
    playHarmonicChord() {
        if (!this.audioContext) return;
        
        try {
            // Get harmonics and pick a subset for the chord
            const harmonics = this.core.getHarmonics();
            if (!harmonics || harmonics.length < 3) return;
            
            // Generate chord from harmonics (use 1st, 3rd, 5th, etc.)
            const chordHarmonics = [0, 2, 4].map(i => 
                harmonics[i] ? harmonics[i] : { ratio: 1, phase: 0 }
            );
            ...
            const oscillators = chordHarmonics.map(harmonic => {
                const osc = this.audioContext.createOscillator();
                const gainNode = this.audioContext.createGain();
